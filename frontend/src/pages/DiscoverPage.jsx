...
export default function DiscoverPage() {
  ...
  if (state.loading) {
    return <section className="page-container discover-page"><h1>发现</h1><p className="state-message">正在整理新的映墨内容…</p></section>
  }

  if (state.error) {
    return <section className="page-container discover-page"><h1>发现</h1><div className="state-message state-message--error"><p>{state.error.message}</p><button onClick={load}>重新加载</button></div></section>
  }
  ...
  return (
    <section className="page-container discover-page">
      <header>
        <p className="eyebrow">非个性化内容目录</p>
        <h1>发现正在发生的事</h1>
        <p>按时间与公开互动整理，轻轻看看别人留下的日常与技巧。</p>
        <Link className="button" to="/search">搜索映墨内容</Link>
      </header>
      ...
      <Section title="最新教材" to="/guides" empty={!data.latest_guides.length}>
        <div className="discover-card-grid">
      ...
      <Section title="热门游戏" to="/games" empty={!data.popular_games.length}>
        <div className="discover-card-grid">
          {data.popular_games.map((item) => <GameCard key={item.id} game={item} />)}
        </div>
      </Section>

      <Section title="活跃创作者" to="/search?scope=user" empty={!data.active_creators.length}>
