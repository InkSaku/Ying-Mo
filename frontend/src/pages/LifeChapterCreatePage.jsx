  ...
  return (
    <section className="life-page page-container">
      ...
      <form className="life-form" onSubmit={submit} noValidate>
        ...
        {!resubmitId && <section className="chapter-cover"><h2>章节封面（可选）</h2><ImageUploadField label="章节封面" purpose="content" currentImageUrl={form.cover_url} onUploaded={async (media) => { if (form.cover_public_id) await deleteUnboundImage(form.cover_public_id).catch(() => {}); setForm((current) => ({ ...current, cover_media_id: media.id, cover_url: media.thumbnail_url, cover_public_id: media.public_id })) }} onRemove={removeCover} disabled={pending} /></section>}
