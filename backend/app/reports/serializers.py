...
def report_dict(report, detail=False):
    data = {"id": report.id, "target_type": report.target_type, "target_id": report.target_id, "reason": report.reason, "status": report.status, "review_round": report.review_round, "assigned_to": public_user_dict(report.assigned_to) if report.assigned_to else None, "created_at": serialize_datetime(report.created_at), "updated_at": serialize_datetime(report.updated_at)}
    if detail:
