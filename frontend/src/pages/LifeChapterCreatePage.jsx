...
import { checkLifeChapterName, createLifeChapter, getLifeChapters } from '../api/life.js'
  ...
  async function submit(event) {
    ...
    try {
      const chapter = await createLifeChapter({
        ...
        description: form.description || null, cover_media_id: form.cover_media_id,
      })
      coverPublicIdRef.current = null
      navigate(`/life/chapter/${chapter.slug}`)
    } catch (requestError) {
  ...
  return (
    <section className="life-page page-container">
      ...
      <form className="life-form" onSubmit={submit} noValidate>
        ...
        <section className="chapter-cover"><h2>章节封面（可选）</h2><ImageUploadField label="章节封面" purpose="content" currentImageUrl={form.cover_url} onUploaded={(media) => setForm((current) => ({ ...current, cover_media_id: media.id, cover_url: media.thumbnail_url, cover_public_id: media.public_id }))} onRemove={removeCover} disabled={pending} /></section>
        {error && <p className="form-feedback form-feedback--error" role="alert">{fieldErrors.cover_media_id || error.message}</p>}
        <button className="button button--primary" disabled={pending || (form.name.trim() && Boolean(check?.exact_match))}>{pending ? '正在创建…' : '创建章节'}</button>
      </form>
    </section>
  )
}
