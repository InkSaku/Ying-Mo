...
export default function GlobalSearch() {
  ...
  return <div className="global-search" ref={root}><form onSubmit={submit}><input value={value} onChange={(event) => setValue(event.target.value)} onFocus={() => items.length && setOpen(true)} onKeyDown={keyDown} placeholder="找日常或点位" aria-label="全站搜索" aria-expanded={open} aria-controls={listId} aria-activedescendant={active >= 0 ? `${listId}-${active}` : undefined} /><button type="submit" aria-label="提交搜索">去找</button></form>{open && items.length > 0 && <div id={listId} className="global-search__suggestions" role="listbox">{items.map((item, index) => <button id={`${listId}-${index}`} role="option" aria-selected={active === index} key={`${item.type}-${item.url}`} type="button" onMouseDown={() => navigate(item.url)}><strong>{item.label}</strong><span>{item.subtitle}</span></button>)}</div>}</div>
}
