...
export function GameCard({ game }) {
  ...
  return <article className="game-card game-card--directory">
    <div className="game-card__media">{cover ? <AdaptiveMedia src={cover} alt={`${game.name_zh}封面`} fit="contain" /> : <CatalogPlaceholder label={game.name_zh} />}</div>
    <div className="game-card__body">
      <header className="game-card__heading">
        <span className="game-card__icon">{icon ? <AdaptiveMedia src={icon} alt="" fit="contain" /> : <CatalogPlaceholder label={game.name_zh} compact />}</span>
...
export function HeroCard({ hero }) {
  const avatar = hero.avatar_thumbnail_url || hero.avatar_url
  return <article className="catalog-card catalog-card--hero"><Link to={`/game/${hero.game.slug}/hero/${hero.slug}`}><div className="catalog-card__media">{avatar ? <AdaptiveMedia src={avatar} alt={`${hero.name_zh} 头像`} fit="cover" /> : <CatalogPlaceholder label={hero.name_zh} />}</div><div><p className="eyebrow">{hero.role || '未标注定位'}</p><h3>{hero.name_zh}</h3>{hero.name_en && <p className="game-card__english">{hero.name_en}</p>}<p>{hero.description || '暂无英雄说明。'}</p></div></Link></article>
}
...
export function MapHeroCard({ hero, gameSlug, mapSlug }) {
  return <article className={`catalog-card map-hero-card ${hero.has_guides ? 'has-guides' : ''}`}>
    <Link to={`/game/${gameSlug}/map/${mapSlug}/hero/${hero.slug}`}>
      <div className="catalog-card__media">{hero.avatar_thumbnail_url || hero.avatar_url ? <AdaptiveMedia src={hero.avatar_thumbnail_url || hero.avatar_url} alt={`${hero.name_zh}头像`} fit="cover" /> : <CatalogPlaceholder label={hero.name_zh} />}</div>
      <div>
