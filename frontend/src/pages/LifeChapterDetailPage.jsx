...
const typeLabels = { city: '城市合集', scenic: '景点合集', travel: '旅行合集', campus: '校园合集', event: '活动合集', custom: '生活合集' }
...
export default function LifeChapterDetailPage() {
  ...
  if (chapterLoading) return <section className="life-page page-container"><p className="state-message">正在打开合集…</p></section>
  if (chapterError) return <section className="life-page page-container"><div className="state-message state-message--error"><p>{chapterError.status === 404 ? '没有找到这个合集。' : chapterError.message}</p><button type="button" onClick={() => setRetry((value) => value + 1)}>重新加载</button></div></section>

  const chapter = chapterState.chapter
  if (chapter.canonical_slug) return <section className="life-page page-container"><p className="state-message">正在前往合并后的合集…</p></section>
  ...
  return (
    <section className="life-page page-container">
      <div className="chapter-hero">
        {chapter.cover_url ? <div className="chapter-hero__media"><AdaptiveMedia src={chapter.cover_url} alt={`${chapter.name} 的合集封面`} fit="contain" loading="eager" /></div> : <div className="chapter-hero__placeholder" aria-hidden="true">映</div>}
        <div>
          <div className="tag-row"><span>{typeLabels[chapter.chapter_type]}</span><span>{chapter.contribution_policy === 'private' ? '仅创建者投稿' : '开放投稿'}</span></div>
          <h1>{chapter.name}</h1>
          ...
          {chapter.contribution_policy === 'private' && !chapter.is_owner && <p className="chapter-policy-note">所有人可浏览，仅合集创建者可投稿。</p>}
          {!chapter.can_post && chapter.contribution_policy === 'public' && <p className="chapter-policy-note">登录且账号具有发布权限后，可以向这个开放合集投稿。</p>}
          <div className="life-toolbar">
            {chapter.can_post && <Link className="button button--primary" to={`/life/create?chapter_id=${chapter.id}`}>在此记录</Link>}
            {chapter.can_edit && <Link className="button" to={admin ? `/admin/chapters/${chapter.id}/edit` : `/life/chapters/${chapter.id}/edit`}>编辑合集</Link>}
            {chapter.can_delete && <button className="button button--danger" type="button" onClick={() => setDeleting(true)}>删除合集</button>}
            {chapter.parent && <Link className="button" to={`/life/chapter/${chapter.parent.slug}`}>返回 {chapter.parent.name}</Link>}
          </div>
        </div>
      </div>
      {chapter.children?.length > 0 && <section className="chapter-children"><h2>子合集</h2><div className="tag-row">{chapter.children.map((child) => <Link key={child.id} to={`/life/chapter/${child.slug}`}>{child.name} · {child.content_count} 条</Link>)}</div></section>}
      <section className="life-section">
        <div className="life-section__heading"><div><p className="eyebrow">合集内容</p><h2>{chapter.name} 的直接记录</h2></div></div>
        ...
        {!postsLoading && !postsError && posts.length === 0 && <p className="life-empty">这个合集还没有内容，写下第一句话或留下一张照片吧。</p>}
